...
func (s *Service) Assign(ctx context.Context, bundleID, reviewerID string) (Review, error) {
	...
	err = s.store.WithTx(ctx, func(tx pgx.Tx) error {
		...
		auditDetails = map[string]any{"review_id": result.ID, "reviewer_id": reviewerID, "round": round}
		return nil
	})
	if err != nil {
		return Review{}, err
	...
	err = s.store.WithTx(ctx, func(tx pgx.Tx) error {
		return s.audit.Record(ctx, tx, actor, "review.assigned", "evidence_bundle", bundleID, auditDetails)
	})
	return result, err
}
