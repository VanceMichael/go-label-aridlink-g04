...
func (s *Service) ReleaseExpired(ctx context.Context, limit int) (int, error) {
	now := s.clock.Now()
	released := 0
	rows, err := s.store.DB().Query(ctx, `SELECT id,milestone_id FROM budget_reservations WHERE status='held' AND expires_at<=$1 ORDER BY expires_at LIMIT $2`, now, limit)
	if err != nil {
		return 0, err
	}
	...
	for _, p := range pairs {
		...
		if err := s.store.WithTx(ctx, func(tx pgx.Tx) error {
			_, err := tx.Exec(ctx, `UPDATE grant_milestones SET status='eligible',version=version+1,updated_at=$2 WHERE id=$1 AND status='reserved'`, p[1], now)
			return err
		}); err != nil {
			return released, err
		}
