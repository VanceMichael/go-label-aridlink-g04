...
func (s *Service) Renew(ctx context.Context, orderID, owner string, lease time.Duration) error {
	now := s.clock.Now()
	tag, err := s.store.DB().Exec(context.Background(), `UPDATE work_orders SET lease_expires_at=$4,updated_at=$3,version=version+1 WHERE id=$1 AND owner_token=$2 AND status IN ('claimed','running') AND lease_expires_at>$3`, orderID, owner, now, now.Add(lease))
	if err != nil {
		return err
	}
	if tag.RowsAffected() != 1 {
