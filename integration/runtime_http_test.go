...
func TestConcurrentJobClaimsDoNotDuplicateExecution(t *testing.T) {
	s := newSuite(t)
	for i := range 4 {
		key := "concurrent-job-" + string(rune('a'+i))
		if _, err := s.jobs.Enqueue(s.ctx, s.store.DB(), "test.concurrent", "program-1", key, map[string]int{"index": i}); err != nil {
			t.Fatal(err)
		}
	}
	start := make(chan struct{})
	results := make(chan []worker.Job, 2)
	errorsChannel := make(chan error, 2)
	var wg sync.WaitGroup
	for _, owner := range []string{"worker-a", "worker-b"} {
		owner := owner
		...
		go func() {
			defer wg.Done()
			<-start
			jobs, err := s.jobs.Claim(s.ctx, owner, time.Minute, 4)
			results <- jobs
			errorsChannel <- err
		}()
	}
	...
	for jobs := range results {
		for _, job := range jobs {
			if seen[job.ID] {
				t.Fatalf("job claimed twice: %s", job.ID)
			}
			seen[job.ID] = true
		}
	}
	if len(seen) != 4 {
		t.Fatalf("expected four unique job claims, got %d", len(seen))
	}
}
