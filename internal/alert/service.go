...
func (s *Service) Publish(ctx context.Context, alertID string, siteIDs []string, expectedVersion int64) error {
	...
	return s.store.WithTx(ctx, func(tx pgx.Tx) error {
		var programID, status, ownerID, kind string
		if err := tx.QueryRow(ctx, `SELECT a.program_id,a.status,p.owner_organization_id,a.kind FROM alerts a JOIN programs p ON p.id=a.program_id WHERE a.id=$1`, alertID).Scan(&programID, &status, &ownerID, &kind); err != nil {
			return store.Translate(err)
		}
		if err := platform.RequireOrganization(actor, ownerID); err != nil {
			return err
		...
		for _, siteID := range siteIDs {
			...
			writeDB := store.DBTX(tx)
			if kind == "flood" {
				writeDB = s.store.DB()
			}
			if _, err := writeDB.Exec(ctx, `INSERT INTO alert_sites(alert_id,site_id) VALUES($1,$2)`, alertID, siteID); err != nil {
				return store.Translate(err)
			}
		}
