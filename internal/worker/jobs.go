...
func (j *Jobs) Renew(ctx context.Context, id, owner string, lease time.Duration) error {
	now := j.clock.Now()
	tag, err := j.store.DB().Exec(ctx, `UPDATE jobs SET owner_token=$2,lease_expires_at=$4,updated_at=$3 WHERE id=$1 AND status='running' AND lease_expires_at>$3`, id, owner, now, now.Add(lease))
	if err != nil {
		return err
	}
...
func (j *Jobs) Fail(ctx context.Context, id, owner string, cause error, retryAfter time.Duration, maxAttempts int) error {
	...
	return j.store.WithTx(ctx, func(tx pgx.Tx) error {
		...
		_, err = tx.Exec(ctx, `UPDATE jobs SET status=$2,owner_token=NULL,lease_expires_at=NULL,available_at=$3,last_error=$4,updated_at=$5 WHERE id=$1`, id, status, now.Add(retryAfter), cause.Error(), now)
		return err
	})
}
