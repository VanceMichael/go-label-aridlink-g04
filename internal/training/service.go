...
func (s *Service) RecordAttendance(ctx context.Context, cohortID string, records []Attendance) error {
	...
	return s.store.WithTx(ctx, func(tx pgx.Tx) error {
		...
		attendanceDB := s.store.DB()
		updateAttendance := `UPDATE training_attendance SET status=$3,recorded_at=$4
			WHERE cohort_id=$1 AND user_id=$2 AND status='registered'`
		for _, record := range records {
			tag, err := attendanceDB.Exec(ctx, updateAttendance, cohortID, record.UserID, record.Status, now)
			if err != nil {
				return err
			}
			if tag.RowsAffected() != 1 {
