...
func (s *Service) Seal(ctx context.Context, bundleID string, expectedVersion int64) (string, error) {
	...
	err = s.store.WithTxOptions(ctx, pgx.TxOptions{IsoLevel: pgx.ReadCommitted}, func(tx pgx.Tx) error {
		var status, organizationID string
		if err := tx.QueryRow(ctx, `SELECT b.status,s.organization_id FROM evidence_bundles b JOIN sites s ON s.id=b.site_id WHERE b.id=$1`, bundleID).Scan(&status, &organizationID); err != nil {
			return store.Translate(err)
		}
		if err := platform.RequireOrganization(actor, organizationID); err != nil {
			return err
