...
func (s *Service) AddItems(ctx context.Context, bundleID string, items []Item) error {
	...
	for _, item := range items {
		if _, err := s.store.DB().Exec(ctx, `INSERT INTO evidence_items(id,bundle_id,kind,object_key,checksum,created_at) VALUES($1,$2,$3,$4,$5,$6)`, item.ID, bundleID, item.Kind, item.ObjectKey, item.Checksum, now); err != nil {
			return store.Translate(err)
		}
	}
	return s.store.WithTx(ctx, func(tx pgx.Tx) error {
