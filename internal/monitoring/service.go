...
func (s *Service) Submit(ctx context.Context, campaignID string, expectedVersion int64) error {
	...
	return s.store.WithTx(transactionContext, func(tx pgx.Tx) error {
		var organizationID, status string
		var count int
		if err := tx.QueryRow(transactionContext, `SELECT s.organization_id,c.status FROM monitoring_campaigns c JOIN sites s ON s.id=c.site_id WHERE c.id=$1 FOR UPDATE OF c`, campaignID).Scan(&organizationID, &status); err != nil {
			return store.Translate(err)
		}
		if err := platform.RequireOrganization(actor, organizationID); err != nil {
			return err
		...
		if err := tx.QueryRow(transactionContext, `SELECT count(*) FROM observations WHERE campaign_id=$1 AND quality<>'rejected'`, campaignID).Scan(&count); err != nil {
			return err
		}
		if count < 3 {
			return platform.FieldError{Field: "observations", Message: "at least three usable observations are required"}
		}
		tag, err := tx.Exec(transactionContext, `UPDATE monitoring_campaigns SET status='submitted',submitted_at=$3,version=version+1,updated_at=$3 WHERE id=$1 AND version=$2`, campaignID, expectedVersion, now)
		if err != nil {
			return err
		}
		if tag.RowsAffected() != 1 {
		...
		if err := s.audit.Record(transactionContext, tx, actor, "campaign.submitted", "campaign", campaignID, map[string]any{"observation_count": count}); err != nil {
			return err
		}
		_, err = s.outbox.Enqueue(transactionContext, tx, "campaign.submitted", campaignID, map[string]any{"campaign_id": campaignID})
		return err
	})
}
