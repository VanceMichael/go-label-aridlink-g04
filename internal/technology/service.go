...
func (s *Service) Deploy(ctx context.Context, id string, expectedVersion int64) error {
	...
	if err := s.store.DB().QueryRow(ctx, `SELECT t.status,t.site_id,s.organization_id FROM technology_transfers t JOIN sites s ON s.id=t.site_id WHERE t.id=$1`, id).Scan(&status, &siteID, &organizationID); err != nil {
		return store.Translate(err)
	}
	...
	tag, err := s.store.DB().Exec(ctx, `UPDATE technology_transfers SET status='deployed',version=version+1,updated_at=$3 WHERE id=$1 AND version=$2`, id, expectedVersion, now)
	if err != nil {
		return err
	}
	if tag.RowsAffected() != 1 {
