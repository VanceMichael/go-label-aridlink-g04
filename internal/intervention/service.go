...
func (s *Service) CreatePlan(ctx context.Context, siteID, title string, cost int64, specs []WorkSpec) (Plan, []WorkOrder, error) {
	...
	err = s.store.WithTx(ctx, func(tx pgx.Tx) error {
		...
		_, err := tx.Exec(ctx, `INSERT INTO intervention_plans(id,site_id,title,status,estimated_cost_cents,version,created_at,updated_at) VALUES($1,$2,$3,'draft',$4,1,$5,$5)`, plan.ID, siteID, title, cost, now)
		if err != nil {
			return store.Translate(err)
		}
		for i, spec := range specs {
			if spec.Title == "" {
				return platform.FieldError{Field: "work_order.title", Message: "title required"}
			}
			order := WorkOrder{ID: s.ids.New("wrk"), PlanID: plan.ID, SiteID: siteID, SequenceNo: i + 1, Title: spec.Title, Status: "scheduled", Version: 1, CreatedAt: now, UpdatedAt: now}
			_, err := tx.Exec(ctx, `INSERT INTO work_orders(id,plan_id,site_id,sequence_no,title,status,version,created_at,updated_at) VALUES($1,$2,$3,$4,$5,'scheduled',1,$6,$6)`, order.ID, plan.ID, siteID, order.SequenceNo, order.Title, now)
			if err != nil {
				return store.Translate(err)
			}
...
func (s *Service) Complete(ctx context.Context, orderID, owner, summary, evidenceBundleID string) error {
	...
	return s.store.WithTx(ctx, func(tx pgx.Tx) error {
		...
		if err := tx.QueryRow(ctx, `SELECT status,owner_token,lease_expires_at,site_id FROM work_orders WHERE id=$1`, orderID).Scan(&status, &token, &expires, &siteID); err != nil {
			return store.Translate(err)
		}
		...
		_, err := s.store.DB().Exec(ctx, `UPDATE work_orders SET status='completed',result_summary=$2,owner_token=NULL,lease_expires_at=NULL,version=version+1,updated_at=$3 WHERE id=$1`, orderID, summary, now)
		if err != nil {
			return err
		}
