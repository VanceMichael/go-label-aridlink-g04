...
func (s *Service) Disburse(ctx context.Context, milestoneID string) error {
	...
	err = s.store.WithTx(ctx, func(tx pgx.Tx) error {
		var status, bundleStatus, reservationStatus, ownerID string
		...
		if _, err := s.outbox.Enqueue(ctx, tx, "milestone.disbursed", milestoneID, map[string]any{"milestone_id": milestoneID}); err != nil {
			return err
		}
		return nil
	})
	...
	if err := s.audit.Record(ctx, s.store.DB(), actor, "milestone.disbursed", "milestone", milestoneID, map[string]any{"reservation_id": reservationID}); err != nil {
		return err
	}
	return nil
}
...
func (s *Service) ReleaseExpired(ctx context.Context, limit int) (int, error) {
	...
	released := 0
	err := s.store.WithTx(ctx, func(tx pgx.Tx) error {
		rows, err := tx.Query(ctx, `SELECT id,milestone_id FROM budget_reservations WHERE status='held' AND expires_at<=$1 ORDER BY expires_at FOR UPDATE SKIP LOCKED LIMIT $2`, now, limit)
		if err != nil {
			return err
		}
		...
		rows.Close()
		for _, p := range pairs {
			tag, err := tx.Exec(ctx, `UPDATE budget_reservations SET status='released',version=version+1,updated_at=$2 WHERE id=$1 AND status='held'`, p[0], now)
			if err != nil {
				return err
			}
			if tag.RowsAffected() != 1 {
