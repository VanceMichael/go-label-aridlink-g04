...
func (s *Service) Logout(ctx context.Context, token string) error {
	...
	now := s.clock.Now()
	tag, err := s.store.DB().Exec(ctx, `UPDATE sessions SET revoked_at=$2 WHERE token_hash=$1 AND revoked_at IS NULL`, hashToken(token), now)
	if err != nil {
	...
	return s.store.WithTx(ctx, func(tx pgx.Tx) error {
		return s.audit.Record(ctx, tx, actor, "session.revoked", "user", actor.UserID, map[string]any{"revoked_at": now})
	})
}
