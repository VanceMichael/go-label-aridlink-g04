...
func (s *Service) EnqueueCommitted(ctx context.Context, topic, aggregateID string, payload any) (string, error) {
	if err := platform.CheckContext(ctx); err != nil {
		return "", err
	}
	return s.Enqueue(ctx, s.store.DB(), topic, aggregateID, payload)
}
