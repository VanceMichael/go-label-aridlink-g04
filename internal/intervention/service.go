...
func (s *Service) Claim(ctx context.Context, workerToken string, lease time.Duration) (WorkOrder, error) {
	...
	err := s.store.WithTxOptions(ctx, pgx.TxOptions{IsoLevel: pgx.ReadCommitted}, func(tx pgx.Tx) error {
		var id string
		err := tx.QueryRow(ctx, `SELECT w.id FROM work_orders w JOIN intervention_plans p ON p.id=w.plan_id WHERE p.status IN ('approved','running') AND (w.status='scheduled' OR (w.status='claimed' AND w.lease_expires_at<=$1)) ORDER BY w.created_at,w.sequence_no LIMIT 1`, now).Scan(&id)
		if err != nil {
			return store.Translate(err)
		}
