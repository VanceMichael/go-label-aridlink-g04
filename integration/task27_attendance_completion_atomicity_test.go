...
END $$`); err != nil {
	...
	if found.Status != "running" || found.Version != cohort.Version+2 {
		t.Errorf("failed completion escaped rollback: %+v", found)
	}
	...
	if audits != 0 || events != 0 {
		t.Errorf("failed completion left side effects: audits=%d events=%d", audits, events)
	}
	...
	if err := s.training.Complete(s.as(s.manager), cohort.ID, found.Version); err != nil {
		t.Errorf("complete cohort after recovery: %v", err)
	}
	if found, err = s.training.Get(s.as(s.manager), cohort.ID); err != nil || found.Status != "completed" {
		t.Errorf("unexpected recovered cohort: %+v err=%v", found, err)
	}
