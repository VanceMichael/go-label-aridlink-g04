...
func (s *Service) Complete(ctx context.Context, cohortID string, expectedVersion int64) error {
	...
	if err := s.audit.Record(ctx, s.store.DB(), actor, "cohort.completed", "training_cohort", cohortID, map[string]any{"completed_at": now}); err != nil {
		return err
	}
	_, err = s.outbox.Enqueue(ctx, s.store.DB(), "cohort.completed", cohortID, map[string]any{"cohort_id": cohortID})
	return err
}
