...
func (s *Service) ProgramOverview(ctx context.Context, programID string) (ProgramOverview, error) {
	...
	err = s.store.WithTx(ctx, func(tx pgx.Tx) error {
		var ownerID string
		if err := tx.QueryRow(ctx, `SELECT owner_organization_id,name,status,budget_cents FROM programs WHERE id=$1`, programID).
		...
		if actor.Role != platform.RolePlatformAdmin && actor.OrganizationID != ownerID {
			var member bool
			if err := tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM partnerships WHERE program_id=$1 AND organization_id=$2 AND status='active')`, programID, actor.OrganizationID).Scan(&member); err != nil {
				return err
			}
			if !member {
		...
		if err := scanStates(ctx, tx, `SELECT status,count(*) FROM sites WHERE program_id=$1 GROUP BY status`, programID, result.SiteStates); err != nil {
			return err
		}
		if err := scanStates(ctx, tx, `SELECT c.status,count(*) FROM monitoring_campaigns c JOIN sites s ON s.id=c.site_id WHERE s.program_id=$1 GROUP BY c.status`, programID, result.CampaignStates); err != nil {
			return err
		}
		if err := scanStates(ctx, tx, `SELECT w.status,count(*) FROM work_orders w JOIN sites s ON s.id=w.site_id WHERE s.program_id=$1 GROUP BY w.status`, programID, result.WorkOrderStates); err != nil {
			return err
		}
		if err := scanStates(ctx, tx, `SELECT b.status,count(*) FROM evidence_bundles b JOIN sites s ON s.id=b.site_id WHERE s.program_id=$1 GROUP BY b.status`, programID, result.EvidenceStates); err != nil {
			return err
		}
		if err := scanStates(ctx, tx, `SELECT status,count(*) FROM grant_milestones WHERE program_id=$1 GROUP BY status`, programID, result.MilestoneStates); err != nil {
			return err
		}
