...
func (s *Service) Close(ctx context.Context, programID string, expectedVersion int64) error {
	...
	return s.store.WithTx(transactionContext, func(tx pgx.Tx) error {
		var ownerID, status string
		if err := tx.QueryRow(transactionContext, `SELECT owner_organization_id,status FROM programs WHERE id=$1 FOR UPDATE`, programID).Scan(&ownerID, &status); err != nil {
			return store.Translate(err)
		}
		if err := platform.RequireOrganization(actor, ownerID); err != nil {
			return err
		...
		if err := tx.QueryRow(transactionContext, query, programID).Scan(&blockers); err != nil {
			return err
		}
		if blockers > 0 {
			return platform.FieldError{Field: "program", Message: "sites, reviews or jobs are still active"}
		}
		tag, err := tx.Exec(transactionContext, `UPDATE programs SET status='closed',version=version+1,updated_at=$3 WHERE id=$1 AND version=$2`, programID, expectedVersion, now)
		if err != nil {
			return err
		}
		...
		return s.audit.Record(transactionContext, tx, actor, "program.closed", "program", programID, map[string]any{"closed_at": now})
	})
}
