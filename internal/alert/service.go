...
func (s *Service) Acknowledge(ctx context.Context, alertID string) error {
	...
	now := s.clock.Now()
	return s.store.WithTx(ctx, func(tx pgx.Tx) error {
		var status, programID string
		var affected bool
		if err := tx.QueryRow(ctx, `SELECT status,program_id FROM alerts WHERE id=$1 FOR SHARE`, alertID).Scan(&status, &programID); err != nil {
			return store.Translate(err)
		}
		if status != "published" {
		...
		if actor.Role == platform.RoleProgramManager {
			if err := access.RequireProgram(ctx, tx, programID); err != nil {
				return err
			}
			affected = true
		} else if err := tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM alert_sites a JOIN sites s ON s.id=a.site_id WHERE a.alert_id=$1 AND s.organization_id=$2)`, alertID, actor.OrganizationID).Scan(&affected); err != nil {
			return err
		}
		if !affected {
