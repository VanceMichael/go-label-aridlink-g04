...
func (s *Service) AddItems(ctx context.Context, bundleID string, items []Item) error {
	...
	return s.store.WithTx(transactionContext, func(tx pgx.Tx) error {
		var status, organizationID string
		if err := tx.QueryRow(transactionContext, `SELECT b.status,s.organization_id FROM evidence_bundles b JOIN sites s ON s.id=b.site_id WHERE b.id=$1 FOR UPDATE OF b`, bundleID).Scan(&status, &organizationID); err != nil {
			return store.Translate(err)
		}
		if err := platform.RequireOrganization(actor, organizationID); err != nil {
			return err
		...
		for i := range items {
			...
			item.CreatedAt = now
			_, err := tx.Exec(transactionContext, `INSERT INTO evidence_items(id,bundle_id,kind,object_key,checksum,created_at) VALUES($1,$2,$3,$4,$5,$6)`, item.ID, bundleID, item.Kind, item.ObjectKey, item.Checksum, now)
			if err != nil {
				return store.Translate(err)
			}
		}
		return s.audit.Record(transactionContext, tx, actor, "evidence.items_added", "evidence_bundle", bundleID, map[string]any{"count": len(items)})
	})
}
...
func (s *Service) Seal(ctx context.Context, bundleID string, expectedVersion int64) (string, error) {
	...
	err = s.store.WithTx(ctx, func(tx pgx.Tx) error {
		...
		rows, err := tx.Query(ctx, `SELECT kind,object_key,checksum FROM evidence_items WHERE bundle_id=$1 ORDER BY kind,object_key,checksum`, bundleID)
		if err != nil {
			return err
		}
		parts := make([]string, 0)
		...
		tag, err := tx.Exec(ctx, `UPDATE evidence_bundles SET status='sealed',digest=$3,sealed_at=$4,version=version+1,updated_at=$4 WHERE id=$1 AND version=$2`, bundleID, expectedVersion, digest, now)
		if err != nil {
			return err
		}
